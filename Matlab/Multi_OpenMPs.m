...
directory = '../Output/';
files = dir(strcat(directory, 'twoSidedBusySend_Multi_OpenMP_Syn_*.txt'));
...
directory = '../Output/';
files = dir(strcat(directory, 'twoSidedBusySend_Multi_OpenMP_Asyn_*.txt'));
for i=1:length(files)
    thisFile = strcat(directory, files(i).name);
    % load file
    data = load(thisFile);
...
xlabel('A: Async, S: Sync, # : Total Number of Running OMP threads');
