...
//To test if Iscatter can match with Scatter
//The answer is: NO
//We have to use either Scatter & Scatter or Iscatter & Iscatter
void test(int sender_address, int my_address, int world_size){
  MPI_Request req;
  double data[world_size];
  double recv_data = 1;
  if (sender_address == my_address){
    MPI_Iscatter(&data, 1, MPI_DOUBLE, &recv_data, 1, MPI_DOUBLE, sender_address, MPI_COMM_WORLD, &req);
  }else{
    MPI_Scatter(&data, 1, MPI_DOUBLE, &recv_data, 1, MPI_DOUBLE, sender_address, MPI_COMM_WORLD);
  }
}
...
void busy_scatter_sync_routine(int sender_address, int my_address, bool isVerbose, int world_size){
  ...
  if (sender_address == my_address){
    for (int i = 0; i < NUM_ACTUAL_MESSAGES; i++){
      ...
      for (int k = 0; k < world_size; k++){
        sendBuffer[i * world_size + k] = current_time;
      }
      //Scatter the buffer
  ...
  if (isVerbose){
    ...
    printf("-----------------------------------------\n");
    printf("Fanout Busy Sync Latency is: \n");
    latency_stats_in_microsecond.print();
    latency_stats_in_microsecond.write_to_csv_file("Output/FanoutBusy_Multi_MPIs_Sync_" + to_string(world_size) + ".txt");
...
void busy_scatter_async_routine(int sender_address, int my_address, bool isVerbose, int world_size){
  ...
  MPI_Request sendRequests[NUM_ACTUAL_MESSAGES];
  ...
  if (sender_address == my_address){
    for (int i = 0; i < NUM_ACTUAL_MESSAGES; i++){
      ...
      //Scatter the buffer
      MPI_Iscatter(&sendBuffer[i * world_size], 1, MPI_DOUBLE, recvBuffer, 1, MPI_DOUBLE, sender_address, MPI_COMM_WORLD, &sendRequests[i]);
    }
    MPI_Waitall(NUM_ACTUAL_MESSAGES, sendRequests, MPI_STATUSES_IGNORE);
